...
if ( $qstatResult !~ /do\s+not\s+exist/ ) {
   ...
   print   "   Its job ID is \"$jobNamePrefix.$sampleID\"; please either remove it from the grid\n";
   print   "   or allow it to complete before retryning this script.\n\n";
...
chomp( my $lastLogFile = `/bin/ls -tr $asmLogDir/$jobNamePrefix.$sampleID.o* | tail -q -n1` );
