...
my $supportScriptPrefix = "$INSTALL_ROOT/unpaired_reads.SGE_GRID/support_scripts";
...
my $localScriptTemplateDir = "$INSTALL_ROOT/unpaired_reads.SGE_GRID/script_templates";
...
if ( -e $locFile ) {
   ...
   while ( chomp( my $line = <IN> ) ) {
      ...
      (my $sampleID, my $fileLoc) = split(/\t/, $line);
...
foreach my $sampleID ( sort { $a cmp $b } keys %$readLocs ) {
   ...
   my $srcFile = $readLocs->{$sampleID};

   system("ln -sf $srcFile $readBase/$sampleID\.fastq");
...
sub promptForReadLocs {
   ...
   while ( not $done ) {
      ...
      while ( length($fileLoc) == 0 ) {
         
         print "Location of FASTQ file for $sampleID: ";
         ...
         if ( $fileLoc =~ /\s/ ) {
            ...
            print "   ERROR: \"$fileLoc\": File not found!\n";
